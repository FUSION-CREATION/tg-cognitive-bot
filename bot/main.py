...
async def cmd_stats(message: Message) -> None:
    ...
    sessions_total = int(progress.get("sessions_total", 0))
    ...
    top_modes = progress.get("top_modes", [])
    mode_counts = progress.get("mode_counts", {})
    challenge = _weekly_challenge(progress, top_modes)
    next_step = _progress_next_step(progress)
    level, inside, to_next = _next_level_progress(sessions_total)
    level_bar = _progress_bar(inside, to_next)
    if top_modes:
        focus_label = ", ".join(f"{label}: {count}" for label, count in top_modes[:3])
    else:
        focus_label = "нет данных"
    ...
    text = "\n\n".join(
        [
            section(
                "📊 Сводка",
                (
                    f"• Уровень: {level} {level_bar} ({inside}/{to_next})\n"
                    f"• Всего сессий: {sessions_total}\n"
                    f"• Серия дней: {streak_days}\n"
                    f"• Активных дней за 7д: {active_days_7d}"
                ),
            ),
            section(
                "🎯 Фокус использования",
                (
                    f"• Сессий за 7д: {sessions_7d}\n"
                    f"• Разборов: {int(mode_counts.get('razbor', 0))}\n"
                    f"• Планов: {int(mode_counts.get('plan', 0))}\n"
                    f"• Проверок: {int(mode_counts.get('audit', 0))}\n"
                    f"• Reality Check: {int(mode_counts.get('reality', 0))}\n"
                    f"• Топ режимы: {focus_label}"
                ),
            ),
            section(
                "🧭 Reality профиль",
            ...
            section("🏁 Челлендж недели", challenge),
