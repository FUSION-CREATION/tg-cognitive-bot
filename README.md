...
- Админ-панель с кнопками + мастер рассылки (сегмент -> контент -> предпросмотр -> подтверждение).
...
- Ручные команды для админа:
  ...
  - `/admin` или `/admin_panel` — панель управления (кнопки)
  - `/admin_cost` — расходы
  ...
  - `/admin_broadcast` — рассылка вручную (текст/фото/фото+подпись/голос/аудио, с предпросмотром и подтверждением)
...
## Ручная рассылка (админ)

1. `/admin` -> `📣 Рассылка`
2. Выбрать сегмент аудитории
3. Прислать контент (текст/фото/голос/аудио)
...
- `VPS_USER` — `root`
- `VPS_PORT` — обычно `22`
- `VPS_PASSWORD` — пароль пользователя `root` на VPS
